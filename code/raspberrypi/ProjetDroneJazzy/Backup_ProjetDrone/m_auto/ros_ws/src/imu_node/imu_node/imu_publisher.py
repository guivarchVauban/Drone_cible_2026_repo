...
class IMUPublisher(Node):
    def __init__(self, calib_samples=200):
        ...
        self.boussole_pub = self.create_publisher(String, 'imu/boussole_bot', 10)
    ...
    def timer_callback(self):
        ...
        # --------- BOUSSOLE ---------
        heading_rad = math.atan2(mag[1], mag[0])
        heading_deg = math.degrees(heading_rad)

        if heading_deg < 0:
            heading_deg += 360

        direction = self.get_direction(heading_deg)

        boussole_msg = String()
        boussole_msg.data = f"{heading_deg:.1f}° - {direction}"
        self.boussole_pub.publish(boussole_msg)
        ...
        print(f"Cap : {heading_deg:.1f}° ({direction})")
        print("--------------------------")
